 ...
 */
class Drive_Cache_Drive_Memcache extends Cache_Drive
{
    ...
    protected function _connect()
    {
        ...
        if (!isset(Cache_Drive_Memcache::$memcaches[$config_name]))
        {

            if (null === Cache_Drive_Memcache::$_memcached_mode)
            {
                if (function_exists('extension_loaded'))
                {
                    ...
                    {
                        Cache_Drive_Memcache::$_memcached_mode = true;
                    }
                    elseif (extension_loaded('memcache'))
                    {
                        Cache_Drive_Memcache::$_memcached_mode = false;
                    }
                    else
                    {
            ...
            if (Cache_Drive_Memcache::$_memcached_mode)
            {
                $memcache = 'memcached';
            }
            else
            ...
            Cache_Drive_Memcache::$memcaches[$config_name] = new $memcache();
            Cache_Drive_Memcache::$memcaches_num[$config_name] = 0;

            if (Cache_Drive_Memcache::$_memcached_mode)
            {
                Cache_Drive_Memcache::$memcaches[$config_name]->addServers($this->servers);
            }
            else
            {
                foreach ($this->servers as $server)
                {
                    ...
                    Cache_Drive_Memcache::$memcaches[$config_name]->addServer($server['host'], $server['port'], (bool)$server['persistent'], $server['weight'], 1, 15, true, 'Cache_Drive_Memcache::failure_addserver');
        ...
        # 设置memcache
        $this->_memcache =& Cache_Drive_Memcache::$memcaches[$config_name];

        Cache_Drive_Memcache::$memcaches_num[$config_name]++;
    }
    ...
    public function close_connect()
    {
        ...
        {
            unset($this->_memcache);
            Cache_Drive_Memcache::$memcaches_num[$this->config_name]--;

            if (0 == Cache_Drive_Memcache::$memcaches_num[$this->config_name])
            {

                if (!Cache_Drive_Memcache::$_memcached_mode)
                {
                    @Cache_Drive_Memcache::$memcaches[$this->config_name]->close();
                }

                if (IS_DEBUG)Core::debug()->info('close memcache server.');

                Cache_Drive_Memcache::$memcaches[$this->config_name] = null;
                unset(Cache_Drive_Memcache::$memcaches[$this->config_name]);
                unset(Cache_Drive_Memcache::$memcaches_num[$this->config_name]);
            }
        }
    }
    ...
    public function get($key)
    {
        ...
        if (Cache_Drive_Memcache::$_memcached_mode && $is_array_key)
        {
    ...
    public static function close_all_connect()
    {
        if (!Cache_Drive_Memcache::$_memcached_mode)
        {
            foreach (Cache_Drive_Memcache::$memcaches as $config_name=>$memcache)
            {
                try
                {
                    $memcache->close();
                ...
                Cache_Drive_Memcache::$memcaches[$config_name] = null;
            }
        }

        # 重置全部数据
        Cache_Drive_Memcache::$memcaches     = array();
        Cache_Drive_Memcache::$memcaches_num = array();
