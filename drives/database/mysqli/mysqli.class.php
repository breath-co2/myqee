 ...
 */
class Drive_Database_Drive_MySQLI extends Database_Drive
{
    /**
     * MySQL使用反引号标识符
     *
    ...
    public function connect($use_connection_type = null)
    {
        ...
        if (!$connection_id || !isset(Database_Drive_MySQLI::$_connection_instance[$connection_id]))
        {
            $this->_connect();
        }
    ...
    public function connection()
    {
        ...
        if ($connection_id && isset(Database_Drive_MySQLI::$_connection_instance[$connection_id]))
        {
            return Database_Drive_MySQLI::$_connection_instance[$connection_id];
        }
        else
        {
    ...
    protected function _connect()
    {
        ...
        while (true)
        {
            ...
            Database_Drive_MySQLI::$_current_connection_id_to_hostname[$_connection_id] = $username .'@'. $hostname .':'. $port;
            ...
            try
            {
                ...
                # 设置实例化对象
                Database_Drive_MySQLI::$_connection_instance[$_connection_id]       = $tmplink;
                # 设置当前连接的数据库
                Database_Drive_MySQLI::$_current_databases[$_connection_id]         = $database;
                # 设置计数器
                Database_Drive_MySQLI::$_connection_instance_count[$_connection_id] = 1;
    ...
    {
        # 检查下是否已经有连接连上去了
        if (Database_Drive_MySQLI::$_connection_instance)
        {
            ...
            foreach ($host_config as $host)
            {
                ...
                if (isset(Database_Drive_MySQLI::$_connection_instance[$_connection_id]))
                {
                    $this->_connection_ids[$this->_connection_type] = $_connection_id;

                    # 计数器+1
                    Database_Drive_MySQLI::$_connection_instance_count[$_connection_id]++;
    ...
    protected function _check_connect()
    {
        ...
        {
            $connection_id = $this->connection_id();
            $connection    = Database_Drive_MySQLI::$_connection_instance[$connection_id];
    ...
    public function close_connect()
    {
        ...
        {
            if ($connection_id && Database_Drive_MySQLI::$_connection_instance[$connection_id])
            {
                if (isset(Database_Drive_MySQLI::$_connection_instance_count[$connection_id]) && Database_Drive_MySQLI::$_connection_instance_count[$connection_id]>1)
                {
                    Database_Drive_MySQLI::$_connection_instance_count[$connection_id]--;
                }
                else
                {
                    $link = Database_Drive_MySQLI::$_connection_instance[$connection_id];
                    $id   = Database_Drive_MySQLI::$_current_connection_id_to_hostname[$connection_id];

                    unset(Database_Drive_MySQLI::$_connection_instance[$connection_id]);
                    unset(Database_Drive_MySQLI::$_connection_instance_count[$connection_id]);
                    unset(Database_Drive_MySQLI::$_current_databases[$connection_id]);
                    unset(Database_Drive_MySQLI::$_current_charset[$connection_id]);
                    unset(Database_Drive_MySQLI::$_current_connection_id_to_hostname[$connection_id]);
                    ...
                    if(IS_DEBUG)Core::debug()->info('close '. $key .' mysqli '. $id .' connection.');
                }
            }
    ...
    public function select_db($database)
    {
        ...
        if (!$connection_id || !isset(Database_Drive_MySQLI::$_current_databases[$connection_id]) || $database!=Database_Drive_MySQLI::$_current_databases[$connection_id])
        {
            $connection = Database_Drive_MySQLI::$_connection_instance[$connection_id];
            ...
            # 记录当前已选中的数据库
            Database_Drive_MySQLI::$_current_databases[$connection_id] = $database;
        }
    }
    ...
    public function set_charset($charset)
    {
        ...
        if (isset(Database_Drive_MySQLI::$_current_charset[$connection_id]) && $charset == Database_Drive_MySQLI::$_current_charset[$connection_id])
        {
            return true;
        }
        ...
        # 记录当前设置的编码
        Database_Drive_MySQLI::$_current_charset[$connection_id] = $charset;
    ...
    public function query($sql, $as_object = null, $connection_type = null)
    {
        ...
        else
        {
            ...
            return new Database_Drive_MySQLI_Result($result, $sql, $as_object, $this->config);
        }
    }
}