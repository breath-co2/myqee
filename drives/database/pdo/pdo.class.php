...
class Drive_Database_Drive_PDO extends Database_Drive
{
    ...
    function __construct(array $config)
    {
        ...
        if ($c)
        {
            ...
            throw new Exception(__('If use pdo Drive, need php 5.4+, because less php 5.4 has the SQL injection vulnerability.'));
        }
        ...
        else
        {
            if (!$type)
            {
                throw new Exception(__('pdo config connection hostname error. need pdo drive type'));
            }
            ...
            if ($type=='uri')
            {
                ...
                if (!$type)
                {
                    throw new Exception(__('pdo :host error. need pdo drive type'), array(':host', $host));
                }
            }
        ...
        if (!in_array($type, Database_Drive_PDO::$_pdo_types))
        {
            throw new Exception(__('Unknown drive type: :type'), array(':type'=>$type));
        }
        $this->_pdo_Drive_type = $type;
    ...
    public function connect($use_connection_type = null)
    {
        ...
        if (!$connection_id || !isset(Database_Drive_PDO::$_connection_instance[$connection_id]))
        {
            $this->_connect();
        }
    ...
    public function connection()
    {
        ...
        if ($connection_id && isset(Database_Drive_PDO::$_connection_instance[$connection_id]))
        {
            return Database_Drive_PDO::$_connection_instance[$connection_id];
        }
        else
        {
    ...
    protected function _connect()
    {
        ...
        while (true)
        {
            ...
            Database_Drive_PDO::$_current_connection_id_to_hostname[$_connection_id] = $dsn;
            ...
            try
            {
                ...
                # 设置实例化对象
                Database_Drive_PDO::$_connection_instance[$_connection_id]       = $tmp_link;
                # 设置当前连接的数据库
                Database_Drive_PDO::$_current_databases[$_connection_id]         = $database;
                # 设置计数器
                Database_Drive_PDO::$_connection_instance_count[$_connection_id] = 1;
    ...
    {
        # 检查下是否已经有连接连上去了
        if (Database_Drive_PDO::$_connection_instance)
        {
            ...
            foreach ($host_config as $host)
            {
                ...
                if (isset(Database_Drive_PDO::$_connection_instance[$_connection_id]))
                {
                    $this->_connection_ids[$this->_connection_type] = $_connection_id;

                    # 计数器+1
                    Database_Drive_PDO::$_connection_instance_count[$_connection_id]++;
    ...
    public function close_connect()
    {
        ...
        {
            if ($connection_id && Database_Drive_PDO::$_connection_instance[$connection_id])
            {
                if (isset(Database_Drive_PDO::$_connection_instance_count[$connection_id]) && Database_Drive_PDO::$_connection_instance_count[$connection_id]>1)
                {
                    Database_Drive_PDO::$_connection_instance_count[$connection_id]--;
                }
                else
                {
                    $id = Database_Drive_PDO::$_current_connection_id_to_hostname[$connection_id];
                    Database_Drive_PDO::$_connection_instance[$connection_id] = null;

                    unset(Database_Drive_PDO::$_connection_instance[$connection_id]);
                    unset(Database_Drive_PDO::$_connection_instance_count[$connection_id]);
                    unset(Database_Drive_PDO::$_current_databases[$connection_id]);
                    unset(Database_Drive_PDO::$_current_charset[$connection_id]);
                    unset(Database_Drive_PDO::$_current_connection_id_to_hostname[$connection_id]);
    ...
    public function select_db($database)
    {
        ...
        if (!$connection_id || !isset(Database_Drive_PDO::$_current_databases[$connection_id]) || $database!=Database_Drive_PDO::$_current_databases[$connection_id])
        {
            /**
             * @var $connection pdo
             */
            $connection = Database_Drive_PDO::$_connection_instance[$connection_id];
    ...
    public function set_charset($charset)
    {
        ...
        if (isset(Database_Drive_PDO::$_current_charset[$connection_id]) && $charset == Database_Drive_PDO::$_current_charset[$connection_id])
        {
            return true;
        }
        ...
        # 记录当前设置的编码
        Database_Drive_PDO::$_current_charset[$connection_id] = $charset;
     ...
     *
     * @param $builder
     * @return Database_Drive_PDO_Result
     */
    public function execute($statement, array $input_parameters = array(), $as_object = null, $connection_type = null)
    {
        ...
        {
            return new Database_Drive_PDO_Result($sth, $this->last_query, $as_object, $this->config);
        }
    }
