 ...
 */
class Drive_Database_Drive_SQLite extends Database_Drive
{
    /**
     * MySQL使用反引号标识符
     *
    ...
    public function connect($use_connection_type = null)
    {
        ...
        if (!$connection_id || !isset(Database_Drive_SQLite::$_connection_instance[$connection_id]))
        {
            $this->_connect();
        }
    ...
    public function connection()
    {
        ...
        if ($connection_id && isset(Database_Drive_SQLite::$_connection_instance[$connection_id]))
        {
            return Database_Drive_SQLite::$_connection_instance[$connection_id];
        }
        else
        {
    ...
    protected function _connect()
    {
        ...
        for ($i=1; $i<=2; $i++)
        {
            ...
            try
            {
                ...
                Database_Drive_SQLite::$_current_connection_id_to_db[$_connection_id] = Core::debug_path($db);
                ...
                # 设置实例化对象
                Database_Drive_SQLite::$_connection_instance[$_connection_id]       = $tmplink;
                # 设置计数器
                Database_Drive_SQLite::$_connection_instance_count[$_connection_id] = 1;
    ...
    {
        # 检查下是否已经有连接连上去了
        if (Database_Drive_SQLite::$_connection_instance)
        {
            $_connection_id = $this->_get_connection_hash($this->config['connection']['db']);

            if (isset(Database_Drive_SQLite::$_connection_instance[$_connection_id]))
            {
                $this->_connection_ids[$this->_connection_type] = $_connection_id;

                # 计数器+1
                Database_Drive_SQLite::$_connection_instance_count[$_connection_id]++;
                return true;
            }
        }
    ...
    protected function _get_connection_hash($file)
    {
        ...
        Database_Drive::$_hash_to_hostname[$hash] = Core::debug_path($file);
    ...
    public function close_connect()
    {
        ...
        {
            if ($connection_id && Database_Drive_SQLite::$_connection_instance[$connection_id])
            {
                if (isset(Database_Drive_SQLite::$_connection_instance_count[$connection_id]) && Database_Drive_SQLite::$_connection_instance_count[$connection_id]>1)
                {
                    Database_Drive_SQLite::$_connection_instance_count[$connection_id]--;
                }
                else
                {
                    $link = Database_Drive_SQLite::$_connection_instance[$connection_id];
                    $id   = Database_Drive_SQLite::$_current_connection_id_to_db[$connection_id];

                    unset(Database_Drive_SQLite::$_connection_instance[$connection_id]);
                    unset(Database_Drive_SQLite::$_current_charset[$connection_id]);
                    unset(Database_Drive_SQLite::$_current_connection_id_to_db[$connection_id]);
    ...
    public function set_charset($charset)
    {
        ...
        $connection_id = $this->connection_id();
        $connection = Database_Drive_SQLite::$_connection_instance[$connection_id];
        ...
        if (isset(Database_Drive_SQLite::$_current_charset[$connection_id]) && $charset==Database_Drive_SQLite::$_current_charset[$connection_id])
        {
            return true;
        }
        ...
        # 记录当前设置的编码
        Database_Drive_SQLite::$_current_charset[$connection_id] = $charset;
    }
     ...
     * @return Database_Drive_SQLite_Result
     */
    public function query($sql, $as_object = null, $use_master = null)
    {
        ...
        else
        {
            ...
            return new Database_Drive_SQLite_Result($result, $sql, $as_object ,$this->config);
        }
    }
}