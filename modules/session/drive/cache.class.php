 ...
 */
class Module_Session_Drive_Cache
{
    ...
    public function __construct($cache_config = null)
    {
        ...
        # 设置前缀
        if ($this->drive()->get_prefix()=='')
        {
            $this->drive()->set_prefix($this->prefix);
        }
    ...
    public function create()
    {
        ...
        if (IS_DEBUG)$this->drive()->session_mode(true);

        $_SESSION = $this->drive()->get($sid);

        if (IS_DEBUG)$this->drive()->session_mode(false);
        ...
        # 将获取的值序列化MD5值
        Session_Drive_Cache::$OLD_SESSION_MD5 = md5(serialize($_SESSION));

        # 当前session id
        Session_Drive_Cache::$Session_ID = $sid;
    }
    ...
    {
        return Session_Drive_Cache::$Session_ID;
    }
    ...
    {
        if (md5(serialize($_SESSION)) != Session_Drive_Cache::$OLD_SESSION_MD5)
        {
            if (!$_SESSION || $_SESSION===array())
            {
                # 清除session数据
                $this->drive()->delete($this->session_id());
            }
            else
            {
                # 如果确实修改则保存
                $this->drive()->set($this->session_id(), $_SESSION, Session::$config['expiration']>0?Session::$config['expiration']:2592000);
            }
        }
    }
}