 ...
 */
class Module_Storage_Drive_Redis extends Storage_Drive
{
    ...
    public function __construct($config_name = 'default')
    {
        ...
        # 增加自动关闭连接列队
        Core::add_close_connect_class('Storage_Drive_Redis');
    }
    ...
    protected function _connect()
    {
        ...
        if (!isset(Storage_Drive_Redis::$redis[$config_name]))
        {
            $class = 'Redis';
            Storage_Drive_Redis::$redis[$config_name] = new $class();
            Storage_Drive_Redis::$redis_num[$config_name] = 0;
            ...
            foreach ($this->servers as $server)
            {
                ...
                {
                    $time   = microtime(1);
                    $status = Storage_Drive_Redis::$redis[$config_name]->$action($server['host'], $server['port'],$server['timeout']);
        ...
        # 设置对象
        $this->_redis = & Storage_Drive_Redis::$redis[$config_name];

        Storage_Drive_Redis::$redis_num[$config_name]++;
    }
    ...
    public function close_connect()
    {
        ...
        {
            unset($this->_redis);
            Storage_Drive_Redis::$redis_num[$this->config_name]--;

            if ( 0 == Storage_Drive_Redis::$redis_num[$this->config_name] )
            {
                @Storage_Drive_Redis::$redis[$this->config_name]->close();

                if (IS_DEBUG)Core::debug()->info('close storage redis server.');

                Storage_Drive_Redis::$redis[$this->config_name] = null;
                unset(Storage_Drive_Redis::$redis[$this->config_name]);
                unset(Storage_Drive_Redis::$redis_num[$this->config_name]);
            }
        }
    }
    ...
    {
        foreach (Storage_Drive_Redis::$redis as $config_name=>$obj)
        {
            try
            {
                $obj->close();
            ...
            Storage_Drive_Redis::$redis[$config_name] = null;
        }

        # 重置全部数据
        Storage_Drive_Redis::$redis = array();
        Storage_Drive_Redis::$redis_num = array();
