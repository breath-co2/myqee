 ...
 */
class Module_Storage_Drive_Swift extends Storage_Drive
{
    /**
     * 授权信息
     *
     ...
     *      (
     *          'drive'        => 'Swift',
     *          'drive_config' => 'https://username:password@localhost:8080/v2.0?region=test&tenant_name=default&warehouses=mytest&prefix=test',
     *      );
    ...
    public function __construct($config_name = 'default')
    {
        ...
        # 增加自动关闭连接列队
        Core::add_close_connect_class('Storage_Drive_Swift');
    }
    ...
    protected function get_response($uri, $method, $headers = array(), $query = array(), $body = null, $chunk_size = 10240)
    {
        ...
        # 计数统计
        Storage_Drive_Swift::$requests_num[$connection_hash]++;

        $metadata = stream_get_meta_data($fp);

        if ((Storage_Drive_Swift::$requests_num[$connection_hash] % 100)===0 || true===$metadata['timed_out'])
        {
            unset($fp);
    ...
    protected function connection($re_connect=false)
    {
        ...
        if (!$re_connect && !isset(Storage_Drive_Swift::$connections[$connection_hash]))
        {
            $fp = $this->fp($this->storage_protocol, $this->storage_host, $this->storage_port, $this->timeout);

            Storage_Drive_Swift::set_connection($connection_hash, $fp);
        }
        else if ($re_connect || (isset(Storage_Drive_Swift::$last_used[$this->storage_host]) && time()-Storage_Drive_Swift::$last_used[$this->storage_host] >= $this->timeout))
        {
            # 超时的连接，销毁后重新连接
            Storage_Drive_Swift::unset_connection($connection_hash);

            return $this->connection($re_connect);
        }

        return Storage_Drive_Swift::$connections[$connection_hash];
    }
            ...
            if (isset($rs['header']['X-Storage-Url']))
            {
                ...
                if ($this->storage_url)
                {
                    ...
                    if ($conection_hash1==$conection_hash2)
                    {
                        ...
                        Storage_Drive_Swift::set_connection($conection_hash1, $fp);
                    }
                    else
                    {
    ...
    {
        foreach (Storage_Drive_Swift::$connections as $host=>$obj)
        {
            try
            {
                fclose($obj);
            ...
            Storage_Drive_Swift::$connections[$host] = null;
        }

        # 重置全部数据
        Storage_Drive_Swift::$connections  = array();
        Storage_Drive_Swift::$last_used    = array();
        Storage_Drive_Swift::$requests_num = array();
    ...
    protected static function set_connection($hash, $fp)
    {
        ...
        if (isset(Storage_Drive_Swift::$connections[$hash]))
        {
            unset(Storage_Drive_Swift::$connections[$hash]);
        }

        Storage_Drive_Swift::$connections[$hash]  = $fp;
        Storage_Drive_Swift::$last_used[$hash]    = time();
        Storage_Drive_Swift::$requests_num[$hash] = 0;
    }
    ...
    protected static function unset_connection($hash)
    {
        if (isset(Storage_Drive_Swift::$connections[$hash]))
        {
            @fclose(Storage_Drive_Swift::$connections[$hash]);
            unset(Storage_Drive_Swift::$connections[$hash]);
        }

        unset(Storage_Drive_Swift::$last_used[$hash]);
        unset(Storage_Drive_Swift::$requests_num[$hash]);
    }
}