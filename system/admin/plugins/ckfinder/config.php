...
$myconfig = Plugins::config();

// LicenseKey : Paste your license key here. If left blank, CKFinder will be
// fully functional, in demo mode.
$config['LicenseName'] = $myconfig['LicenseName'];
$config['LicenseKey'] = $myconfig['LicenseKey'];
...
*/
$baseUrl = Myqee::config('core.upload.urlpath');
...
ATTENTION: The trailing slash is required.
*/
$baseDir = UPLOADPATH;//resolveUrl($baseUrl);
...
*/
$config['RoleSessionVar'] = Myqee::config('core.session.name');
//session_start();
...
*/
$config['AccessControl'][] = $myconfig['AccessControl'];/* Array(
		'role' => '*',
		'resourceType' => '*',
		'folder' => '/',
...
$config['ResourceType'][] = Array(
		'name' => '今日上传',				// Single quotes not allowed
		'url' => $baseUrl . date("Y/m/d"),
		'directory' => $baseDir .date("Y/m/d"),
		'maxSize' => 0,
...
$config['ResourceType'][] = Array(
		'name' => '本月上传',				// Single quotes not allowed
		'url' => $baseUrl . date("Y/m"),
		'directory' => $baseDir .date("Y/m"),
		'maxSize' => 0,
		'allowedExtensions' => '7z,aiff,asf,avi,bmp,csv,doc,fla,flv,gif,gz,gzip,jpeg,jpg,mid,mov,mp3,mp4,mpc,mpeg,mpg,ods,odt,pdf,png,ppt,pxd,qt,ram,rar,rm,rmi,rmvb,rtf,sdc,sitd,swf,sxc,sxw,tar,tgz,tif,tiff,txt,vsd,wav,wma,wmv,xls,zip',
		'deniedExtensions' => '');

$config['ResourceType'][] = Array(
		'name' => '全部文件',				// Single quotes not allowed
		'url' => $baseUrl . '',
		'directory' => $baseDir . '',
		'maxSize' => 0,
		'allowedExtensions' => '7z,aiff,asf,avi,bmp,csv,doc,fla,flv,gif,gz,gzip,jpeg,jpg,mid,mov,mp3,mp4,mpc,mpeg,mpg,ods,odt,pdf,png,ppt,pxd,qt,ram,rar,rm,rmi,rmvb,rtf,sdc,sitd,swf,sxc,sxw,tar,tgz,tif,tiff,txt,vsd,wav,wma,wmv,xls,zip',
...
*/
$config['FilesystemEncoding'] = $myconfig['FilesystemEncoding'];
