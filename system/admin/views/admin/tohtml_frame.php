...
//	ajax.setVar("myTextBox", '123');
	...
	ajax.onCompletion = function (){
		...
		if (typeof info == 'object') {
			...
			if (info.classid>0){
				updatadoing('toinfobyclassid',info.classid,info.thedoingpage+'/'+info.allcount,0,info.runtime);
			}
...
function updatadoing(type,classid,count,offset,runtime){
	if (type =='toinfobyclassid'){
		$obj_count = $('do_count_'+classid);
		$obj_time = $('do_time_'+classid);
		if (!$obj_count || !$obj_time)return;
		$obj_count.innerHTML = count;

		if (runtime>0){
			runtime = parseFloat(runtime);
			if (doingTime['classid_'+classid]>0){
				doingTime['classid_'+classid] += runtime;
			}else{
				doingTime['classid_'+classid] = runtime;
			}
			$obj_time.innerHTML = doingTime['classid_'+classid].toFixed(4);
		}else{
...
if (is_array($does)){
	...
	foreach ($does as $item){
		...
		echo '
<tr'.($i % 2 == 0 ? ' class="td3"':'').'>
<td class="td2" id="do_info_'.$item['classid'].'">'.$item['info'].'</td>
<td class="td2" id="do_count_'.$item['classid'].'" align="center">?</td>
<td class="td2" id="do_time_'.$item['classid'].'" align="center">排队中...</td>
</tr>';
	}
}
?>
...
<?php
if ($type =='info'){
	echo '正在生成栏目ID：'.$classinfo['classid'].'“'.$classinfo['classname'].'”内容！';
}
?>
<script>
//document.location.href ="<?php echo $tohtmlurl?>";
</script>
</body>
</html>
</textarea>
<script type="text/javascript">
//window['mainFrame'].document.write($('myhtml').value);
