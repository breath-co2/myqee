 ...
 *
 * $Id: Kohana.php,v 1.3 2009/08/06 06:12:18 jonwang Exp $
 *
 * @package    Core
 * @author     Kohana Team
...
final class Kohana {
	...
	public static function controller_setup(){
		static $run;

		// This function can only be run once
		if ($run === TRUE)
			return;
		if (!Myqee::$controller)return;
		
		
		Myqee::$controller -> input = Input::instance();
		self::$instance =& Myqee::$controller;
		Router::setup();
	...
	public static function setup()
	{
		...
		self::$include_paths = Myqee::include_paths(TRUE);
		
		if (!Myqee::$controller){
			Event::add('controller.loaded',array('Kohana','controller_setup'));
		}else{
			Myqee::$controller -> input = Input::instance();
			self::$instance =& Myqee::$controller;
		}
