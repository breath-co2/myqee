 ...
 */
    ...
    function __construct($tplname,$group=null){
    	$this -> Smarty($tplname,$group);
    }
    
    function Smarty($tplname,$group=null)
    {
    	if ($group===true){
    		//临时模板
    		...
    		//创建临时文件
    		Tools::createfile($this -> template_dir.$this -> _template ,$tplname);
    	...
    	$this -> cache_dir = MYAPPPATH .'cache/';
    	
	    $this->assign('SCRIPT_NAME', isset($_SERVER['SCRIPT_NAME']) ? $_SERVER['SCRIPT_NAME']
                    : @$GLOBALS['HTTP_SERVER_VARS']['SCRIPT_NAME']);
    }
    
    function render($print = FALSE,$data=null)
    {
    	...
    	if ($print){
    		...
    		$data = $this -> fetch($this->_template);
    	}
    	if ($this -> _istemptpl == true){
    		//清除编译后的文件
    		$this -> clear_compiled_tpl($this -> _template);
    		//删除临时模板所产生的目录及文件
    		Tools::remove_dir($this -> template_dir);
    	}
    	return $data;
    }
