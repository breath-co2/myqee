...
class arr_Core {
	 ...
	 * 对于二维数组进行排序
	 * @param $array
	 * @param $sort_flags 同asort的$sort_flags
	 * @param $arrkey 二维排序的key
	 * @return boolean
	 */
	public static function asort(array & $array = null , $sort_flags = null , $arrkey = 'myorder' ){
		if (!is_array($array))return false;
		$newarr = array();
		foreach ($array as $key => $item){
			$newarr[$key] = isset($item[$arrkey])?$item[$arrkey]:var_export($item,true);
		}
		$r = asort($newarr,$sort_flags);
