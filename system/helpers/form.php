...
class form_Core {
	...
	public static function uploadfile($data, $value = '', $extra = '' , $uptype = null , $allownum = 1){
		...
		return form::input($data , $value , $extra .' class="input" style="width:300px;"' ) . ' <img src="' . ADMIN_IMGPATH. '/admin/'.$uptype.'.gif" align="absmiddle" style="cursor:pointer" onclick="show_upload_frame(\'' . $data['id'] . '\',\''. url::site('uploadfile/inframe/'.$uptype.'/'.$allownum) .'\',\''.$uptype.'\')" />';
	}
	...
	public static $option_html = '';
	public static function _listclass($list,$selectclassid,$isshowcolor = false,$setcolorby='',$spacer=''){
		if($list){
	...
	public static function htmlarea($data,$value='',$extra='',$textconfig='',$autoload=true){
		...
		return '<span style="display:none;">' .form::textarea($data,$value,$extra) . '<input id="'.$thename.'___Config" value="BaseHref=http://'.Myqee::config('core.mysite_domain').'/&'.$textconfig.'" style="display:none;" type="hidden" /></span><iframe name="'.$thename.'___Frame" id="'.$thename.'___Frame" '.($autoload?'src':'onclick="this.src=this.title;this.title=\'\';this.onclick=null;" title').'="'.ADMIN_IMGPATH.'/admin/fckeditor/editor/fckeditor.html?InstanceName='.$thename.'&Toolbar='.($data['toolbar']?$data['toolbar']:'Default').'" frameBorder="no" width="'.($data['size']?($data['size']*6):'100%').'" scrolling="no" height="'.($data['rows']?($data['rows']*12+60):'350').'"></iframe>';
	}
...
function paixu_htmlarea(){
	...
	tmphtml +=\'<table border="0" cellpadding="2" style="width:580px;border-top:none;" cellspacing="1" class="tableborder"><tr><td class="td1"><input type="button" value="上移选定" onclick="if (!myTable[\\\'mytable\\\'])return;myTable[\\\'mytable\\\'].up(1);" class="btn" /><input type="button" value="下移选定" class="btn" onclick="if (!myTable[\\\'mytable\\\'])return;myTable[\\\'mytable\\\'].down();" /></td><td width="180" class="td1" align="center"><input type="button" value="取消" onclick="closeMsgBox()" class="btns" align="asbmiddle" /><input type="button" value="修改排序" onclick="htmlorder_seveorder()" title="修改完排序需要点此按钮才会保存排序！" class="bbtn" /></td></tr></table></div>\';
	tmphtml +=\'<script defer="defer" type="text/javascript">\';
	tmphtml +=\'function htmlorder_seveorder(){var tmphtml="";var objs = $("myEditorTable").rows;var lis=frameFrame.$("pageUlDiv").getElementsByTagName("li");for(var i=0;i<objs.length-1;i++){if (!lis[i])continue;var theid=objs[(i+1)].getElementsByTagName("input")[1].id.substr("15");lis[i].id="pagehtmlareaTitle_"+theid;if(frameFrame.nowEditId==theid){lis[i].className="now"}else{lis[i].className="";}};frameFrame.html_rename_all();closeMsgBox();}\';	tmphtml +=\'</\';
	tmphtml +=\'script>\';
...
function do_del_htmlarea(delid){
	...
	if (objdiv){
		if(ie){
			...
			var objarea = window.$("htmlarea_info[content]_"+delid);
			if (objarea){
				objarea.name=null;
...
<script type="/images/admin/fckeditor/fckeditor.js"></script>
</div>
...
<div class="clear"></div>
	...
	public static function timeinput($data,$value='', $extra = '' ,$showinput = true){
	$thename = $data['name']?$data['name']:$data;
		...
		$tmpinput = '<span style="white-space:nowrap;">'.form::input($data,$value==''?'':date("Y-m-d".($data['time']?' H:i:s':''),$value>0?$value:$_SERVER['REQUEST_TIME']),'onclick="showcalendar(event,this,'.($data['time']?'true':'false').');" onfocus="showcalendar(event, this,'.($data['time']?'true':'false').');if(this.value==\'0000-00-00'.($data['time']?' 0:0:0':'').'\')this.value=\'\';" '.$extra) ;
	...
	public static function changeinput($data,$value='',$extra='',$options=array(),$array_unshift,$selected=null,$extra2=''){
		if ( ! is_array($data))
		{
			$data = array('name' => $data);
		}
		...
		return form::input($data,$value,$extra) .' '. form::dropdown(array(),$options,$selected,'onchange="$(\''.$data['id'].'\').value=this.value;" '.$extra2);
	}
	...
	public static function textarea($data, $value = '', $extra = '')
	{
		...
		if (!$data['class'])$data['class']='input';
	...
	public static function ajaxinput($data,$ajaxurl='',$handler='null',$method='GET',$errorhandler='null'){
		$handler or $handler='null';
		$errorhandler or $errorhandler='null';
		//$keyup = '_ajax_input_fun(event,this,"'.$ajaxurl.'",function(){('.$handler.')();},"'.$method.'","'.$errorhandler.'")';
		if (is_array($data)){
			//$data['onkeyup'] = $keyup;
		}else{
			$data = array(
		...
		$data['id'] or $data['id'] = '_'.Tools::get_rand(18);
//		$data['focus'] = 'if(this.value=="")return;_ajax_input_show(this)';
//		$data['blur'] = '_ajax_input_hidden(this)';
		
		
		$html = str_replace("\n			",'','
			<script type="text/javascript">
				$import("'.SITE_URL.'images/js/ajax.js","var ajax=new sack();");
				$import("'.SITE_URL.'images/js/ajaxinput.js");
			</script>
			'.
			form::input($data)
			.'
			<script type="text/javascript">
				ajaxinput("'.$data['id'].'","'.$ajaxurl.'",'.$handler.',"'.$method.'",'.$errorhandler.');
			</script>');
		return $html;
	}
}