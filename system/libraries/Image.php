...
class Image_Core {
	...
	public function __construct($image, $config = NULL)
	{
		...
		if ($check === FALSE)
			throw new Error_Exception('image.getimagesize_missing');

		// Check to make sure the image exists
		if ( ! is_file($image))
			throw new Error_Exception('image.file_not_found', $image);
		...
		if ( ! is_array($image_info) OR count($image_info) < 3)
			throw new Error_Exception('image.file_unreadable', $image);

		// Check to make sure the image type is allowed
		if ( ! isset(Image::$allowed_types[$image_info[2]]))
			throw new Error_Exception('image.type_not_allowed', $image);
		...
		// Load configuration
		$this->config = (array) $config + Myqee::config('core.image');

		// Set driver class name
		$driver = 'Image_'.ucfirst($this->config['driver']).'_Driver';

		// Load the driver
		if ( ! Myqee::auto_load($driver))
			throw new Error_Exception('core.driver_not_found', $this->config['driver'], get_class($this));
		...
		if ( ! ($this->driver instanceof Image_Driver))
			throw new Error_Exception('core.driver_implements', $this->config['driver'], get_class($this), 'Image_Driver');
	}
	...
	public function __get($property)
	{
		...
		{
			throw new Error_Exception('core.invalid_property', $property, get_class($this));
		}
	}
	...
	public function resize($width, $height, $master = NULL)
	{
		if ( ! $this->valid_size('width', $width))
			throw new Error_Exception('image.invalid_width', $width);

		if ( ! $this->valid_size('height', $height))
			throw new Error_Exception('image.invalid_height', $height);

		if (empty($width) AND empty($height))
			throw new Error_Exception('image.invalid_dimensions', __FUNCTION__);
		...
		elseif ( ! $this->valid_size('master', $master))
			throw new Error_Exception('image.invalid_master');
	...
	public function crop($width, $height, $top = 'center', $left = 'center')
	{
		...
		if ( ! $this->valid_size('top', $top))
			throw new Error_Exception('image.invalid_top', $top);

		if ( ! $this->valid_size('left', $left))
			throw new Error_Exception('image.invalid_left', $left);
	...
	public function flip($direction)
	{
		if ($direction !== Image::HORIZONTAL AND $direction !== Image::VERTICAL)
			throw new Error_Exception('image.invalid_flip');
	...
	public function save($new_image = FALSE, $chmod = 0644, $keep_actions = FALSE)
	{
		...
		if ( ! is_writable($dir))
			throw new Error_Exception('image.directory_unwritable', $dir);
	...
	public function watermark($config=null,$autosave = TRUE){
		//print_r($config);
		if (is_array($config)){
			$config = arr::merge(Myqee::config('core.watermark'),$config);
		}else{
			$config = Myqee::config('core.watermark');
		}
		$this -> actions['watermark'] = $config;
		$this -> actions['quality'] = $config['quality']>0 && $config['quality']<=100 ? $config['quality'] : 70;
		if ($autosave){
			return $this -> save($this->image['file']);
		}else{
			return $this;
		}
	}
